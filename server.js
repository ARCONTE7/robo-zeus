...
// Mapeamento de planos para IDs do painel
const PLANOS_IDS = {
    'Mensal': '1',
    'Bimestral': '9',
    'Trimestral': '2',
...
// 📝 FUNÇÃO PARA SALVAR TUDO (MONITORAMENTO)
...
app.post('/webhook', async (req, res) => {
    ...
    if (texto === 'renovar' || buttonId === 'renovar_agora') {
        await enviarLista(from, "🔄 RENOVAÇÃO DE PLANO\n\nEscolha seu plano para renovar:", "Ver Planos", [{
    ...
    if (buttonId) {
        if (buttonId === 'm_teste') {
            ...
            await enviarTexto(from, "👨‍💻 SUPORTE TÉCNICO\n\nUm atendente humano foi notificado, se for urgente mandar uma mensagem para 91992192289. Pode descrever seu problema!\n\n(Digite MENU a qualquer momento para voltar ao robô)");
        }
        return res.sendStatus(200);
    }
...
👉 Digite MENU para voltar.`;
        ...
        if (listId.startsWith('plano_')) {
            ...
            if (!testes[from]) {
                await enviarTexto(from, "Olá! Faça o *teste gratuito de 6 horas* primeiro!");
                await menuDispositivos(from);
            } else {
                const planos = {
                ...
                if (planoEscolhido) {
                    ...
                    if (cobranca && cobranca.url) {
                        ...
                        clientes[from].installmentId = installmentId;
                        clientes[from].statusPagamento = 'AGUARDANDO';
                        salvarJSON(CLIENTES_FILE, clientes);

                        let tentativas = 0;
                        console.log('Iniciando monitoramento de pagamento para: ' + from);

                        const intervaloMonitoramento = setInterval(async () => {
                            tentativas++;
                            console.log('[' + tentativas + '/60] Verificando pagamento ' + from + '...');

                            if (tentativas > 60) {
                                console.log('Timeout de verificacao atingido (10 minutos)');
                            ...
                            if (statusPago.includes(status)) {
                                ...
                                const clientesAtualizados = lerJSON(CLIENTES_FILE);
                                if (clientesAtualizados[from]) {
                                    clientesAtualizados[from].statusPagamento = 'PAGO';
                                    clientesAtualizados[from].dataPagamento = moment().toISOString();
                                    clientesAtualizados[from].dataVencimento = moment().add(planoEscolhido.dias, 'days').toISOString();
                                    clientesAtualizados[from].aviso1h = false;
                                    clientesAtualizados[from].aviso24h = false;
                                    clientesAtualizados[from].avisoVencido = false;
                                    salvarJSON(CLIENTES_FILE, clientesAtualizados);
                                }

                                const testesSalvos = lerJSON(TESTES_FILE);
                                if (testesSalvos[from] && testesSalvos[from].user) {
                                    const usuario = testesSalvos[from].user;
                                    const senha = testesSalvos[from].pass;
                                    const planoId = PLANOS_IDS[planoEscolhido.nome] || '1';

                                    console.log('Executando renovacao para usuario: ' + usuario);
                                    const renovado = await executarRenovacao(usuario, planoId, planoEscolhido.nome);

                                    if (renovado) {
                                        await enviarTexto(from, '🎉 PAGAMENTO CONFIRMADO!\n\n✅ Seu plano ' + planoEscolhido.nome + ' foi ativado!\n\n📺 Suas credenciais:\nUsuário: ' + usuario + '\nSenha: ' + senha + '\n\n📅 Vencimento:\n' + moment().add(planoEscolhido.dias, 'days').format('DD/MM/YYYY HH:mm') + '\n\n🙏 Obrigado pela confiança!\n\n*Equipe HelpTV* 🔱');
                                    } else {
                                        await enviarTexto(from, '🎉 PAGAMENTO CONFIRMADO!\n\n✅ Seu plano ' + planoEscolhido.nome + ' foi confirmado!\n\n⚠️ Ocorreu um erro na renovação automática.\n\nEntre em contato com o suporte!\n\n*Equipe HelpTV* 🔱');
                                    }
                                } else {
                                    await enviarTexto(from, '🎉 PAGAMENTO CONFIRMADO!\n\n✅ Seu plano ' + planoEscolhido.nome + ' foi confirmado!\n\n📅 Novo Vencimento:\n' + moment().add(planoEscolhido.dias, 'days').format('DD/MM/YYYY HH:mm') + '\n\n🙏 Obrigado pela confiança!\n\n*Equipe HelpTV* 🔱');
                                }
...
📅 Validade: ${planoEscolhido.dias} dias

🔗 Clique no link abaixo para pagar via PIX:
${cobranca.url}

✨ Após o pagamento, favor enviar print do comprovante.

━━━━━━━━━━━━━━━━━━━━
👉 Digite MENU para voltar.`);
                    } else {
                        await enviarTexto(from, "❌ Erro ao gerar pagamento. Tente novamente ou fale com o suporte.");
                    }
                }
            }
        }
